...
test('guards pass on the real release manifest + plan', () => {
  ...
  // publishOrder resolves to all seven units in deterministic (core-first) order.
  assert.deepEqual(resolveOrder(plan).map((o) => o.unit),
    ['core', 'dashboard-image', 'cli', 'k8s-module', 'operator-image', 'operator-chart', 'k8s-docs']);
});
...
test('idempotent: rerun skips already-published artifacts', { skip }, () => {
  ...
  assert.equal(first.counts.published, 7, 'first run publishes all seven units');
  assert.equal(first.counts.complete, 7);
  assert.equal(second.counts.published, 0, 'rerun publishes nothing');
  assert.equal(second.counts.complete, 7, 'rerun sees all seven already-published');
...
test('resumable: rerun resumes from a non-complete ledger', { skip }, () => {
  ...
  assert.equal(resumed.counts.complete, 7);
  ...
  proof('publish-resume.txt',
    ['# PROOF - resumable: rerun resumes from the first non-complete ledger entry', '',
      ...
      `RUN 2 (resume + verify):  ${line(resumed)}   <- resumable=2, published=0, complete=7`].join('\n'));
});
