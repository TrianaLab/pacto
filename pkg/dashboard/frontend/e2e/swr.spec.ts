...
// explicitly, through the one seam every API call passes: `window.fetch`, which the
// demo's boot.js shadows to route into the wasm engine. Playwright's own network
// interception never sees a wasm-served fetch, so it cannot be used here.
