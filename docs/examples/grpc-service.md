...
- **`type: grpc` with `contract`** — the `.proto` file is bundled in the OCI artifact, so the API contract travels with the service version (see [interfaces](../contract-reference.md#interfaces) for the supported types)
- **Health on gRPC** — a `grpc` health interface needs no `path`; the service is expected to implement the [gRPC Health Checking Protocol](https://github.com/grpc/grpc/blob/master/doc/health-checking.md) (see [runtime.health](../contract-reference.md#runtimehealth))
