...
Every transcript below was produced by running the command shown above it. They
are regenerated by `make gen-demo-transcripts` and compared against the committed
tree by `make docs-check`, so a change in what the CLI prints fails a gate rather
than quietly making this page a work of fiction. The same beats run as an
acceptance test in `tests/acceptance/local/demo-arc.sh`, which asserts on the
bytes rather than on exit codes.

## Before you start

Everything here is offline. No cluster, no registry, no running service and no
network — the fixture is committed to the repository, so the whole arc works on a
plane.
...
Here is one of their contracts — `payments-service` at 2.0.0, the version beat 5
refuses:
...
consumer's declared version range does not admit 2.0.0. A consumer graded
...
The consumer list is identical. What is new is the last two lines: two of those
consumers are not hypothetical, they are running right now, so the command exits
non-zero. Beat 5 refused a change because of what it is. This refuses it because
of where it lands.
...
## Beat 11 — Hand an agent read tools, and nothing else
...
| 11 | An agent gets read-only tools by default, and the withholding is announced |
